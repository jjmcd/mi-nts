...
$q2 = "SELECT MIN(`period`),MAX(`period`),MIN(`qtr`),MAX(`qtr`) "
  . "FROM `netreport` WHERE `netid`=" . $netid ;
$r2=getResult($q2,$db);
$limits = getRow( $r2, $db );

// Initialize the database query
$q3 = "SELECT `period`,`qtr` FROM `netreport` WHERE `netid`=" .
...
ImageLine( $image, 379, 0, 379, 239, $dark);
ImageLine( $image, 379, 239, 0, 239, $dark);
ImageLine( $image, 0, 239, 0, 0, $dark);
	...
	if ( $x[$i] == $test )
	  {
	    ...
	    ImageString( $image, 2, $xl, 215, $yr, $dark );
	  }
      }
  }
for ( $i=1; $i<16; $i+=2 )
  {
    ...
    if ( $xl >= $gl-2 && $xr <= $gr+2 )
      {
	ImageFilledRectangle($image, $xl, 215, $xr, 228, $gray);
	ImageString( $image, 2, $xl+22, 215, $year, $white );
      }
...
ImageString($image, 1, 220, 230, 'netQTR.php $Revision: 1.1 $', $ltgray );
