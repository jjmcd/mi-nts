...
//    pshri.php
//    $Revision: 1.8 $ - $Date: 2008-01-22 16:52:55-05 $
//
...
//
{
    ...
    $SQL="SELECT `period`,`cat1`,`cat2`,`cat3`,`cat4`,`cat5`,`cat6`," .
	"`total`,`comment`,`updated` " .
    ...
    while ($myrow = getRow($result,$db) )
	{
	    if ( $myrow[9] > $maxdate )
		$maxdate = $myrow[9];
	    $SQL2 = "SELECT `lastday` FROM `periods` WHERE `periodno` = " . 
		$myrow[0];
