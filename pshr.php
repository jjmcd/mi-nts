<?php
//    index.php
//    $Revision: 1.11 $ - $Date: 2008-01-22 16:43:44-05 $
//
//    Displays the Public Service Honor Roll results for the specified
//    period.  If no period specified displays latest period.
//
{
...
<?php
    ...
    echo "    <p><h1>Public Service Honor Roll<br>for " . 
	convertDate(singleResult($SQL,$db)) . "</h1></p>\n";

    // Get the actual report data for this period
    $SQL='SELECT `call`,`cat1`,`cat2`,`cat3`,`cat4`,`cat5`,`cat6`,`total`,'
	.'`comment`,`updated` FROM `pshr` WHERE `period`='
        . $period . ' ORDER BY `total` DESC';
    $result=getResult($SQL,$db);

    echo "    <table>\n";
    echo "      <th>Call<th>Nets<th>Tfc<th>Appt<th>Plan<th>Emrg<th>Digi"
	. "<th>Total<th>Comments<tr>\n";
    ...
    while ($myrow = getRow($result,$db) )
	{
	    ...
	    $callrow='<a id="OsRowL" href="pshri.php?call=' . $myrow[0] 
		.'">' . $myrow[0] . '</A>';
